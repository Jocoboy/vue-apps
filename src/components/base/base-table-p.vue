<template>
    <div style="height: 100%; width: 100%" class="flex-column">
        ...
        <el-table ref="baseTableRef" v-loading="isLoading" class="flex-1" border stripe element-loading-text="加载中..."
            v-bind="$attrs" :data="isPage ? pageRes.records : tableDataList" size="default"
            :header-cell-style="{ background: '#F8F9FA', color: '#333333' }" highlight-current-row :row-key="rowKey"
            :selection="true" :reserve-selection="true">
        ...
        <div v-if="isPage" class="flex-end-center" style="margin-top: 10px">
            <el-config-provider :locale="zhCn">
                <el-pagination v-if="pageRes && pageRes.total > 0" :current-page="pageParams.pageNum"
                    :page-size="pageParams.pageSize" :total="pageRes.total == null ? 0 : pageRes.total" :small="true"
                    :page-sizes="[10, 20, 30, 40, 50]" :background="true"

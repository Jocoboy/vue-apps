<template>
    <div>
        <el-upload v-bind="$attrs" v-model:file-list="fileList" :action="uploadUrl" list-type="picture-card"
            :headers="{ [tokenObj.tokenName]: tokenObj.tokenValue, TENANT_ID: customTenantId ? customTenantId : tenantId }"
            :before-upload="beforeUpload" :on-success="onSuccess" :on-remove="onRemove" :on-preview="onPreview"
            multiple>
            <el-icon>
                <Plus />
            </el-icon>
